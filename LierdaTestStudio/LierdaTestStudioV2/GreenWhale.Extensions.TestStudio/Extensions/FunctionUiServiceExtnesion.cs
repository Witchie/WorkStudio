...
namespace GreenWhale.Extensions.TestTools2.Extensions
{
    public static class FunctionUiServiceExtnesion
    {
        ...
        public static FunctionUIService UseTestStudio(this FunctionUIService functionUIService)
        {
            ...
            functionUIService.AddClickRibbonMenuWithPage<ProtocalGenerator>(new RibbonMenuWithPageView("生产中心/协议管理/数据包生成器:生成数据包"));
            //functionUIService.AddClickRibbonMenuWithPage<FactoryProduceView>(new RibbonMenuWithPageView("生产中心/生产记录/测试记录查询:查询生产测试信息"));
            functionUIService.AddClickRibbonMenuWithPages<FactoryProduceView, ScanPanelView>(new RibbonMenuWithPageView("生产中心/工厂生产/生产测试:生产并测试表具"), new RibbonMenuWithPageView("生产中心/工厂生产/扫描面板:扫描二维码"));
